...
export async function messagingRoutes(app:FastifyInstance, config:AppConfig){
  ...
  app.post("/api/v1/messages", async (req, reply)=>{
    ...
    // Transaction boundary (§33): message + deliveries insert atomically.
    // Enqueue happens after COMMIT; on enqueue failure rows stay QUEUED and
    // recoverable via manual retry (no partial fan-out, no silent loss).
    ...
    try{
      ...
      if(idempotencyKey){
        const existing = await client.query("SELECT id FROM messages WHERE tenant_id=$1 AND api_key_id=$2 AND idempotency_key=$3",[tenantId, ak.id, idempotencyKey]);
        if(existing.rows.length){
          const mid=existing.rows[0].id;
          const dels=await client.query("SELECT id,destination_id,status FROM deliveries WHERE message_id=$1 AND tenant_id=$2",[mid, tenantId]);
        ...
        msgId=id("msg");
        const ins=await client.query("INSERT INTO messages (id,tenant_id,api_key_id,idempotency_key,subject,body,metadata_json) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (tenant_id,api_key_id,idempotency_key) DO NOTHING RETURNING id",[msgId, tenantId, ak.id, idempotencyKey, body.message.subject ?? null, body.message.body, body.message.metadata? JSON.stringify(body.message.metadata): null]);
        if(!ins.rows.length){
          const won=await client.query("SELECT id FROM messages WHERE tenant_id=$1 AND api_key_id=$2 AND idempotency_key=$3",[tenantId, ak.id, idempotencyKey]);
          const mid=won.rows[0].id;
          const dels=await client.query("SELECT id,destination_id,status FROM deliveries WHERE message_id=$1 AND tenant_id=$2",[mid, tenantId]);
          await client.query("ROLLBACK");
          return reply.status(200).send(success({ id: mid, status:"queued", deliveries: dels.rows.map((r:any)=>({ id:r.id, destination_id:r.destination_id, status:r.status })) }, String(req.id)));
        }
        ...
        msgId=id("msg");
        await client.query("INSERT INTO messages (id,tenant_id,api_key_id,idempotency_key,subject,body,metadata_json) VALUES ($1,$2,$3,$4,$5,$6,$7)",[msgId, tenantId, ak.id, null, body.message.subject ?? null, body.message.body, body.message.metadata? JSON.stringify(body.message.metadata): null]);
      }
      deliveries=[];
      for(const destId of body.destinations){
        const dest = destRows.rows.find((r:any)=>r.id===destId);
        const dlvId=id("dlv");
        await client.query("INSERT INTO deliveries (id,tenant_id,message_id,provider_connection_id,destination_id,status) VALUES ($1,$2,$3,$4,$5,'QUEUED')",[dlvId, tenantId, msgId, dest.provider_connection_id, destId]);
    ...
    // last-used only after the request passed auth + IP + rate gates
    await pool.query("UPDATE api_keys SET last_used_at=NOW() WHERE id=$1", [ak.id]);
    for(const d of deliveries){
      try{ await enqueueDelivery(config, d.id); }catch(e:any){
        req.log.error({ err:e, deliveryId: d.id }, "enqueue failed after commit; delivery stays QUEUED for manual retry");
        return reply.status(503).send(errorBody("INTERNAL_ERROR", "Message persisted but queue unavailable; retry delivery via manual retry.", String(req.id)));
      }
    }
  ...
  app.post("/api/v1/tenants/:tenantId/messages", async (req, reply)=>{
    ...
    if(destRows.rows.length !== body.destinations.length) return reply.status(404).send(errorBody("NOT_FOUND","One or more destinations not found.",String(req.id)));
    for(const d of destRows.rows) if(d.status!=="active") return reply.status(422).send(errorBody("VALIDATION_ERROR",`Destination ${d.id} is disabled.`,String(req.id)));
    const dashClient = await pool.connect();
    const msgId=id("msg");
    const deliveries:any[]=[];
    try{
      await dashClient.query("BEGIN");
      await dashClient.query("INSERT INTO messages (id,tenant_id,subject,body,metadata_json) VALUES ($1,$2,$3,$4,$5)",[msgId, tenantId, body.message.subject ?? null, body.message.body, body.message.metadata? JSON.stringify(body.message.metadata): null]);
      for(const destId of body.destinations){
        const dest = destRows.rows.find((r:any)=>r.id===destId);
        const dlvId=id("dlv");
        await dashClient.query("INSERT INTO deliveries (id,tenant_id,message_id,provider_connection_id,destination_id,status) VALUES ($1,$2,$3,$4,$5,'QUEUED')",[dlvId, tenantId, msgId, dest.provider_connection_id, destId]);
        deliveries.push({ id: dlvId, destination_id: destId, status:"QUEUED" });
      }
      await dashClient.query("COMMIT");
    }catch(e:any){
      try{ await dashClient.query("ROLLBACK"); }catch{}
      throw e;
    }finally{
      dashClient.release();
    }
  ...
  // Additive per-message delivery summary (DESIGN §23) so the UI never
  // reduces mixed delivery states to a misleading single status.
  app.get("/api/v1/tenants/:tenantId/messages", async (req, reply)=>{
    ...
    const where:string[]=["m.tenant_id=$1"]; const vals:any[]=[tenantId];
    if(q.q){ where.push(`(m.subject ILIKE $2 OR m.body ILIKE $2 OR m.id ILIKE $2)`); vals.push(`%${q.q}%`); }
    const baseWhere=where.join(" AND ");
    const countSql = `SELECT COUNT(*) FROM messages m WHERE ${baseWhere}`;
    const total=(await pool.query(countSql, vals)).rows[0].count;
    const offset=(page-1)*per;
    const dataSql = `SELECT m.*,
        COALESCE(s.delivery_total,0)::int AS delivery_total,
        COALESCE(s.delivered_count,0)::int AS delivered_count,
        COALESCE(s.failed_count,0)::int AS failed_count
      FROM messages m LEFT JOIN (
        SELECT message_id, COUNT(*) AS delivery_total,
          COUNT(*) FILTER (WHERE status='DELIVERED') AS delivered_count,
          COUNT(*) FILTER (WHERE status IN ('FAILED','DEAD')) AS failed_count
        FROM deliveries WHERE tenant_id=$1 GROUP BY message_id
      ) s ON s.message_id=m.id
      WHERE ${baseWhere} ORDER BY m.created_at DESC LIMIT $${vals.length+1} OFFSET $${vals.length+2}`;
  ...
  app.get("/api/v1/tenants/:tenantId/deliveries/:dlvId", async (req, reply)=>{
    ...
    const attempts=await pool.query("SELECT * FROM delivery_attempts WHERE delivery_id=$1 AND tenant_id=$2 ORDER BY attempt_number",[dlvId, tenantId]);
  ...
  app.post("/api/v1/tenants/:tenantId/deliveries/:dlvId/retry", async (req, reply)=>{
    ...
    // respect provider/destination status before requeue (§65)
    const gate=await pool.query(`SELECT pc.status AS conn_status, d.status AS dest_status FROM deliveries dl JOIN provider_connections pc ON pc.id=dl.provider_connection_id AND pc.tenant_id=dl.tenant_id JOIN destinations d ON d.id=dl.destination_id AND d.tenant_id=dl.tenant_id WHERE dl.id=$1 AND dl.tenant_id=$2`,[dlvId, tenantId]);
    if(!gate.rows.length) return reply.status(409).send(errorBody("CONFLICT","Delivery references unavailable.",String(req.id)));
    if(gate.rows[0].conn_status!=="active" || gate.rows[0].dest_status!=="active") return reply.status(422).send(errorBody("VALIDATION_ERROR","Provider connection or destination is disabled.",String(req.id)));
    const moved=await pool.query("UPDATE deliveries SET status='QUEUED', next_retry_at=null, updated_at=NOW() WHERE id=$1 AND tenant_id=$2 AND status IN ('FAILED','DEAD','RETRYING')",[dlvId, tenantId]);
    if(!moved.rowCount) return reply.status(409).send(errorBody("CONFLICT","Delivery not eligible for retry.",String(req.id)));
    try{
      await enqueueDelivery(config, dlvId);
    }catch(e:any){
      req.log.error({ err:e, deliveryId: dlvId }, "manual retry enqueue failed; delivery stays QUEUED");
      return reply.status(503).send(errorBody("INTERNAL_ERROR","Retry queued in DB but queue unavailable; delivery stays QUEUED.",String(req.id)));
    }
