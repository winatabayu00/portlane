...
function Webhooks({ tenantId }:{tenantId:string}){
  ...
  const [form,setForm]=useState({name:"",forwarding_url:""}); const [edit,setEdit]=useState<any|null>(null); const [editForm,setEditForm]=useState({name:"",forwarding_url:"",status:"active"});
  ...
  async function create(e:React.FormEvent){ e.preventDefault(); await apiFetch(`/api/v1/tenants/${tenantId}/webhook-endpoints`,{method:"POST",body:JSON.stringify({name:form.name,forwarding_url:form.forwarding_url||undefined})}); setForm({name:"",forwarding_url:""}); reload(); }
...
function Settings({ tenantId }:{tenantId:string}){
  ...
  async function addIp(e:React.FormEvent){ e.preventDefault(); if(!ipFor) return; try{ await apiFetch(`/api/v1/tenants/${tenantId}/api-keys/${ipFor}/ip-allowlist`,{method:"POST",body:JSON.stringify(ipForm)}); setIpForm({cidr:"",description:""}); openIp(ipFor); }catch(er:any){ setMsg(er.message); } }
  async function delIp(entryId:string){ if(!ipFor) return; await apiFetch(`/api/v1/tenants/${tenantId}/api-keys/${ipFor}/ip-allowlist/${entryId}`,{method:"DELETE"}); openIp(ipFor); }
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    {msg && <div style={{fontSize:12,padding:"8px 12px",borderRadius:8,background:"var(--danger-soft)",border:"1px solid rgba(239,68,68,0.2)",color:"var(--danger)"}}>{msg}</div>}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:12}}>
      {keys.map((k:any)=><div key={k.id} className="pl-card" style={{display:"flex",flexDirection:"column",gap:8}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontWeight:600,fontSize:14}}>{k.name}</span><StatusBadge status={k.status}/></div><div className="pl-mono" style={{fontSize:12,background:"var(--bg-input)",border:"1px solid var(--border)",padding:"6px 8px",borderRadius:6}}>{k.key_prefix}••••••••</div><div style={{fontSize:11,color:"var(--text-muted)"}}>Last used {k.last_used_at? new Date(k.last_used_at).toLocaleString():"never"}</div><div style={{display:"flex",gap:8,marginTop:4}}>{k.status==="active" && <button className="pl-btn pl-btn-secondary pl-btn-sm" onClick={()=>revoke(k.id)}>Revoke</button>}<button className="pl-btn pl-btn-ghost pl-btn-sm" onClick={()=>openIp(k.id)}>IP Allowlist</button></div></div>)}
    ...
    {ipFor && (
      <div className="pl-overlay" onClick={()=>setIpFor(null)}>
        <div className="pl-modal" onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>{ipList.length===0? <div style={{fontSize:12,color:"var(--text-muted)"}}>No entries — all IPs allowed. Add a CIDR to restrict.</div> : ipList.map((r:any)=><div key={r.id} style={{display:"flex",gap:8,alignItems:"center",padding:"8px 10px",background:"var(--bg-input)",border:"1px solid var(--border)",borderRadius:8}}><span className="pl-mono" style={{fontSize:12,flex:1}}>{r.cidr}</span><span style={{fontSize:11,color:"var(--text-muted)"}}>{r.description ?? ""}</span><button className="pl-btn pl-btn-ghost pl-btn-sm" style={{color:"var(--danger)"}} onClick={()=>delIp(r.id)}><I.trash/></button></div>)}</div>
        </div>
      </div>
    )}
...
export default function App(){
  ...
  async function createTenant(e:React.FormEvent){
    ...
    try{ const j=await apiFetch("/api/v1/tenants",{method:"POST",body:JSON.stringify({name:newTenantName})}); setTenants([...tenants, j.data]); setTenantId(j.data.id); setNewTenantName(""); setShowTenantCreate(false); setTenantOpen(false); location.reload(); }catch(er:any){ setTenantMsg(er.message); }
  }
